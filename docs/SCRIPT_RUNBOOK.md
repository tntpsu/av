...
- **Collision + frame changes (2026-09-22, T-ACC-SCORER-COLLISION-BLIND / T-ACC-RADAR-FRAME):** Card 3 prints `Physical Contact: N frame(s)` from `vehicle/lead_collision_detected`, and Card 5 now counts those frames as collisions (`n_collision`, `n_contact_frames` in the score dict) — `ACC_SCORE_COLLISION_FORCES_ZERO` finally fires. The `Collision Frames (distance < 0)` line is kept for schema compatibility but is structurally always 0 (Unity's override clamps the range to 0.1 m). Near-miss (Card 3 and Card 5) is computed on the bumper gap `distance − ACC_RADAR_RANGE_OFFSET_M` (4.43 m; `radar_fwd_distance_m` is centre-to-centre). `tools/drive_summary_core.py` near-miss uses the same offset. **Post-convergence gap RMSE vs IDM equilibrium (2026-09-26, T-ACC-RMSE-GATE):** Card 2 prints `Post-conv RMSE vs EQ` — RMSE of (gap − `acc_idm_equilibrium_gap_m`) from the first ACC frame within max(2 m, 20 %) of equilibrium, gated at `ACC_GAP_RMSE_POST_CONV_GATE_M` (10 m) — plus the standing bias (gap − EQ) and EQ/s*. `compute_post_convergence_gap(d)` is the pure function; the score dict carries it under `post_convergence_gap` (informational, not in the composite). The legacy whole-run `Gap Error RMSE` vs s* stays printed for continuity but is not a scenario gate. Frame is era-aware: `recording_provenance.radar_range_offset_m` (0 for pre-09-22 files) → correction = `ACC_RADAR_RANGE_OFFSET_M − offset_recorded` (the recorded `radar_fwd_distance_m` is the sensor's filtered gap AFTER the offset, orchestrator.py:9760). E-stop events exclude `EMERGENCY_BRAKE` frames (B1 reflex, not e-stops); near-miss excludes standstill (speed < 0.5). Historical G2/G1/H5 composites re-score to 0 — see `tools/nightly/acc-sweep/PROMPT.md` § Scoring changes.
