...
| **T-JERK-MEAS** | **Reconcile interframe-extrapolation jerk output with analyzer measurement** | 2026-04-18 finding: on hairpin_15, `pp_max_steering_jerk=18.0` is enforced (P99 observed jerk = 18.00 exactly), but MAX observed jerk = 30.08 on 4-9 outlier frames/run. At those peaks, `pp_steering_jerk_limited=0` (limiter did not fire). Hypothesis: interframe extrapolation runs control at ~30 Hz between camera frames (~11 Hz on hairpin); the jerk limiter operates on per-control-step dt, but the analyzer computes jerk across Unity camera frames. Multi-update aggregation across camera intervals produces measured jerk above the limiter's per-step guarantee. Impact: hairpin Control layer stuck at 80 (overall score 95.7, all-layers-≥95 goal unmet). Score delta to fix: ~+4 pts (Control 80 → 95+). Scope: CODE PATCH in interframe-extrapolation chain OR clarify scoring metric to match limiter's time domain. See `docs/agent/current_state.md` "Session 2026-04-18" for full diagnostic chain. **Do not tune `pp_max_steering_jerk` further — it's already working correctly for 99% of frames.** Start investigation at `control/pid_controller.py:6626` (universal output jerk limiter — also uses dt not dt²) and `av_stack/orchestrator.py` interframe-update loop. |
