    ...
    async loadSummary() {
        ...
        try {
            ...
            if (transportContract && typeof transportContract === 'object' && String(transportContract.availability || '').toLowerCase() === 'available') {
                ...
                const fvFrameDelta = transportContract.front_vehicle_frame_delta || {};
                const frontAge = transportContract.front_age_ms || {};
                const vehicleAge = transportContract.vehicle_age_ms || {};
                const coherencePassRate = Number(transportContract.coherence_pass_rate);
                const completeButIncoherentRate = Number(transportContract.complete_but_incoherent_rate);
                const sourceContextQueue = transportContract.source_context_queue_depth || {};
                const sourceContextFrameDelta = transportContract.source_context_frame_delta || {};
                const sourceContextTimeDelta = transportContract.source_context_time_delta_ms || {};
                ...
                html += `<tr><td>Join Failure / Side / Selected Contract:</td><td style="text-align: right;">${this.escapeHtml(String(transportContract.join_failure_reason_mode || 'none'))} / ${this.escapeHtml(String(transportContract.join_failure_side_mode || 'none'))} / ${this.escapeHtml(String(transportContract.selected_failure_contract_reason_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Selected Stage / Bundle Close:</td><td style="text-align: right;">${this.escapeHtml(String(transportContract.selected_failure_source_stage_mode || 'none'))} / ${this.escapeHtml(String(transportContract.source_bundle_close_reason_mode || 'none'))}</td></tr>`;
                ...
                html += `<tr><td>Coherence Pass / Complete-but-Incoherent / Reason:</td><td style="text-align: right;">${Number.isFinite(coherencePassRate) ? coherencePassRate.toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(completeButIncoherentRate) ? completeButIncoherentRate.toFixed(1) + '%' : 'N/A'} / ${this.escapeHtml(String(transportContract.coherence_reason_mode || 'coherent'))}</td></tr>`;
                ...
                html += `<tr><td>Front ↔ Vehicle Δframe (P50/P95/Max):</td><td style="text-align: right;">${Number.isFinite(Number(fvFrameDelta.p50)) ? Number(fvFrameDelta.p50).toFixed(1) : 'N/A'} / ${Number.isFinite(Number(fvFrameDelta.p95)) ? Number(fvFrameDelta.p95).toFixed(1) : 'N/A'} / ${Number.isFinite(Number(fvFrameDelta.max)) ? Number(fvFrameDelta.max).toFixed(1) : 'N/A'}</td></tr>`;
                html += `<tr><td>Front / Vehicle Age (P95):</td><td style="text-align: right;">${Number.isFinite(Number(frontAge.p95)) ? Number(frontAge.p95).toFixed(1) + ' ms' : 'N/A'} / ${Number.isFinite(Number(vehicleAge.p95)) ? Number(vehicleAge.p95).toFixed(1) + ' ms' : 'N/A'}</td></tr>`;
                html += `<tr><td>Source Context Queue / Δframe / Δt (P95):</td><td style="text-align: right;">${Number.isFinite(Number(sourceContextQueue.p95)) ? Number(sourceContextQueue.p95).toFixed(1) : 'N/A'} / ${Number.isFinite(Number(sourceContextFrameDelta.p95)) ? Number(sourceContextFrameDelta.p95).toFixed(1) : 'N/A'} / ${Number.isFinite(Number(sourceContextTimeDelta.p95)) ? Number(sourceContextTimeDelta.p95).toFixed(1) + ' ms' : 'N/A'}</td></tr>`;
                html += `<tr><td>Bundle Deadline / Age / Inflight (P95):</td><td style="text-align: right;">${Number.isFinite(Number((transportContract.source_bundle_deadline_ms || {}).p95)) ? Number((transportContract.source_bundle_deadline_ms || {}).p95).toFixed(1) + ' ms' : 'N/A'} / ${Number.isFinite(Number((transportContract.source_bundle_age_ms || {}).p95)) ? Number((transportContract.source_bundle_age_ms || {}).p95).toFixed(1) + ' ms' : 'N/A'} / ${Number.isFinite(Number((transportContract.source_bundle_inflight_count || {}).p95)) ? Number((transportContract.source_bundle_inflight_count || {}).p95).toFixed(1) : 'N/A'}</td></tr>`;
                html += `<tr><td>Bundle Vehicle Built / Enqueued / Sent:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.source_bundle_vehicle_state_built_rate)) ? Number(transportContract.source_bundle_vehicle_state_built_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_bundle_vehicle_state_enqueued_rate)) ? Number(transportContract.source_bundle_vehicle_state_enqueued_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_bundle_vehicle_state_sent_rate)) ? Number(transportContract.source_bundle_vehicle_state_sent_rate).toFixed(1) + '%' : 'N/A'}</td></tr>`;
                html += `<tr><td>Bundle Camera Requested / Sent / Superseded:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.source_bundle_camera_requested_rate)) ? Number(transportContract.source_bundle_camera_requested_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_bundle_camera_sent_rate)) ? Number(transportContract.source_bundle_camera_sent_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_bundle_superseded_before_send_rate)) ? Number(transportContract.source_bundle_superseded_before_send_rate).toFixed(1) + '%' : 'N/A'}</td></tr>`;
                html += `<tr><td>Camera Request Attempted / Accepted / Rejected:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.source_camera_request_attempted_rate)) ? Number(transportContract.source_camera_request_attempted_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_camera_request_accepted_rate)) ? Number(transportContract.source_camera_request_accepted_rate).toFixed(1) + '%' : 'N/A'} / ${this.escapeHtml(String(transportContract.source_camera_request_rejected_reason_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Camera Request Skipped / Disposition:</td><td style="text-align: right;">${this.escapeHtml(String(transportContract.source_camera_request_skipped_reason_mode || 'none'))} / ${this.escapeHtml(String(transportContract.source_camera_request_disposition_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Active Eligible / Debug-Unbundled / Contract:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.active_camera_eligible_rate)) ? Number(transportContract.active_camera_eligible_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.debug_unbundled_capture_rate)) ? Number(transportContract.debug_unbundled_capture_rate).toFixed(1) + '%' : 'N/A'} / ${this.escapeHtml(String(transportContract.camera_capture_contract_reason_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Bundle Abort / Vehicle Send Blocked:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.source_bundle_aborted_before_vehicle_send_rate)) ? Number(transportContract.source_bundle_aborted_before_vehicle_send_rate).toFixed(1) + '%' : 'N/A'} / ${Number.isFinite(Number(transportContract.source_vehicle_send_blocked_by_camera_request_rate)) ? Number(transportContract.source_vehicle_send_blocked_by_camera_request_rate).toFixed(1) + '%' : 'N/A'} / ${this.escapeHtml(String(transportContract.source_bundle_abort_reason_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Active Excluded / Reason / UnbundledEntered:</td><td style="text-align: right;">${Number.isFinite(Number(transportContract.active_camera_excluded_event_rate)) ? Number(transportContract.active_camera_excluded_event_rate).toFixed(1) + '%' : 'N/A'} / ${this.escapeHtml(String(transportContract.active_camera_excluded_reason_mode || 'none'))} / ${Number.isFinite(Number(transportContract.unbundled_camera_entered_active_path_rate)) ? Number(transportContract.unbundled_camera_entered_active_path_rate).toFixed(1) + '%' : 'N/A'}</td></tr>`;
            ...
            if (accComfort && typeof accComfort === 'object' && String(accComfort.availability || '').toLowerCase() === 'available') {
                ...
                html += `<tr><td>Actual / Simple / Dynamic Gap (P50):</td><td style="text-align: right;">${Number.isFinite(Number(accComfort.actual_gap_p50_m)) ? Number(accComfort.actual_gap_p50_m).toFixed(2) : 'N/A'} / ${Number.isFinite(Number(accComfort.target_gap_p50_m)) ? Number(accComfort.target_gap_p50_m).toFixed(2) : 'N/A'} / ${Number.isFinite(Number(accComfort.dynamic_gap_p50_m)) ? Number(accComfort.dynamic_gap_p50_m).toFixed(2) : 'N/A'} m</td></tr>`;
                ...
                html += `<tr><td>Dynamic / Eq Gap Error P50:</td><td style="text-align: right;">${Number.isFinite(Number(accComfort.dynamic_gap_error_p50_m)) ? Number(accComfort.dynamic_gap_error_p50_m).toFixed(2) : 'N/A'} / ${Number.isFinite(Number(accComfort.equilibrium_gap_error_p50_m)) ? Number(accComfort.equilibrium_gap_error_p50_m).toFixed(2) : 'N/A'} m</td></tr>`;
                ...
                html += `<tr><td>Track / Close / Policy / Lead / Eq / Detect / Compressed:</td><td style="text-align: right;">${Number.isFinite(Number(accComfort.tracking_rate)) ? Number(accComfort.tracking_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.closing_rate)) ? Number(accComfort.closing_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.policy_limited_tracking_rate)) ? Number(accComfort.policy_limited_tracking_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.lead_limited_tracking_rate)) ? Number(accComfort.lead_limited_tracking_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.equilibrium_limited_tracking_rate)) ? Number(accComfort.equilibrium_limited_tracking_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.detection_limited_following_rate)) ? Number(accComfort.detection_limited_following_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accComfort.compressed_rate)) ? Number(accComfort.compressed_rate).toFixed(1) : 'N/A'}%</td></tr>`;
                html += `<tr><td>Lead Speed / Closure Reserve (P50):</td><td style="text-align: right;">${Number.isFinite(Number(accComfort.lead_speed_estimate_p50_mps)) ? Number(accComfort.lead_speed_estimate_p50_mps).toFixed(2) : 'N/A'} / ${Number.isFinite(Number(accComfort.closure_reserve_p50_mps)) ? Number(accComfort.closure_reserve_p50_mps).toFixed(2) : 'N/A'} m/s</td></tr>`;
                html += `<tr><td>Convergence Issue Reason:</td><td style="text-align: right;">${this.escapeHtml(String(accComfort.convergence_issue_reason_mode || 'none'))}</td></tr>`;
            ...
            const accDetection = summary.acc_detection_contract || null;
            if (accDetection && typeof accDetection === 'object' && String(accDetection.availability || '').toLowerCase() === 'available') {
                const issueDetected = Boolean(accDetection.issue_detected);
                html += '<div id="summary-section-acc-detection-contract" style="background: #2a2a2a; padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem; border-left: 4px solid #4a90e2;">';
                html += '<h3 style="margin-top: 0; color: #4a90e2;">ACC Detection Contract</h3>';
                html += '<table style="width: 100%; color: #e0e0e0;">';
                html += `<tr><td>Issue Detected:</td><td style="text-align: right; color:${issueDetected ? '#ff6b6b' : '#4caf50'};">${issueDetected ? 'YES' : 'NO'}</td></tr>`;
                html += `<tr><td>Detection / Stable / Recent Loss:</td><td style="text-align: right;">${Number.isFinite(Number(accDetection.detection_rate_pct)) ? Number(accDetection.detection_rate_pct).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accDetection.stable_detection_rate_pct)) ? Number(accDetection.stable_detection_rate_pct).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(accDetection.recent_detection_loss_rate_pct)) ? Number(accDetection.recent_detection_loss_rate_pct).toFixed(1) : 'N/A'}%</td></tr>`;
                html += `<tr><td>Loss Events / Max No-Detect Run:</td><td style="text-align: right;">${Number(accDetection.detection_loss_event_count || 0)} / ${Number(accDetection.no_detect_run_length_max || 0)}</td></tr>`;
                html += `<tr><td>Issue Mode / Tuning Valid:</td><td style="text-align: right;">${this.escapeHtml(String(accDetection.issue_mode || 'none'))} / ${accDetection.convergence_tuning_valid ? 'YES' : 'NO'}</td></tr>`;
                html += '</table>';
                html += '</div>';
            }
            ...
            if (leadContinuity && typeof leadContinuity === 'object' && String(leadContinuity.availability || '').toLowerCase() === 'available') {
                ...
                html += `<tr><td>Assoc Eligible / Track Active / Source:</td><td style="text-align: right;">${Number.isFinite(Number(leadContinuity.association_eligible_rate_pct)) ? Number(leadContinuity.association_eligible_rate_pct).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(leadContinuity.track_active_rate_pct)) ? Number(leadContinuity.track_active_rate_pct).toFixed(1) : 'N/A'}% / ${this.escapeHtml(String(leadContinuity.track_source_mode || 'none'))}</td></tr>`;
                html += `<tr><td>Raw / Assoc / Hold:</td><td style="text-align: right;">${Number.isFinite(Number(leadContinuity.raw_detect_rate_pct)) ? Number(leadContinuity.raw_detect_rate_pct).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(leadContinuity.same_lane_association_rate_pct)) ? Number(leadContinuity.same_lane_association_rate_pct).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(leadContinuity.continuity_hold_rate_pct)) ? Number(leadContinuity.continuity_hold_rate_pct).toFixed(1) : 'N/A'}%</td></tr>`;
                html += `<tr><td>Hold Reason / Drop / Age P95:</td><td style="text-align: right;">${this.escapeHtml(String(leadContinuity.continuity_hold_reason_mode || 'none'))} / ${this.escapeHtml(String(leadContinuity.continuity_drop_reason_mode || 'none'))} / ${Number.isFinite(Number(leadContinuity.continuity_hold_age_p95_ms)) ? Number(leadContinuity.continuity_hold_age_p95_ms).toFixed(1) : 'N/A'} ms</td></tr>`;
                html += '</table>';
                html += '</div>';
            }
            ...
            if (highwayMildCurve && typeof highwayMildCurve === 'object' && String(highwayMildCurve.availability || '').toLowerCase() === 'available') {
                ...
                const armDeficit = highwayMildCurve.curve_local_arm_phase_deficit || {};
                const armEffect = highwayMildCurve.curve_local_arm_effect_score || {};
                ...
                html += `<tr><td>Scenario Family:</td><td style="text-align: right;">${this.escapeHtml(String(highwayMildCurve.scenario_family_mode || 'unknown'))}</td></tr>`;
                ...
                html += `<tr><td>Active Authority Active / Speed-Gated / Reason:</td><td style="text-align: right;">${Number.isFinite(Number(highwayMildCurve.active_mild_curve_authority_active_on_high_error_rate)) ? Number(highwayMildCurve.active_mild_curve_authority_active_on_high_error_rate).toFixed(1) : 'N/A'}% / ${Number.isFinite(Number(highwayMildCurve.active_mild_curve_authority_speed_gated_on_high_error_rate)) ? Number(highwayMildCurve.active_mild_curve_authority_speed_gated_on_high_error_rate).toFixed(1) : 'N/A'}% / ${this.escapeHtml(String(highwayMildCurve.mpc_active_mild_curve_authority_reason_mode_on_high_error || 'N/A'))}</td></tr>`;
                html += `<tr><td>Active Authority Weight / Ratio / Speed / Curvature / Gate (P50):</td><td style="text-align: right;">${Number.isFinite(Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_weight_active_state || {}).p50)) ? Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_weight_active_state || {}).p50).toFixed(3) : 'N/A'} / ${Number.isFinite(Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_ratio_active_state || {}).p50)) ? Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_ratio_active_state || {}).p50).toFixed(3) : 'N/A'} / ${Number.isFinite(Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_speed_weight || {}).p50)) ? Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_speed_weight || {}).p50).toFixed(3) : 'N/A'} / ${Number.isFinite(Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_curvature_weight || {}).p50)) ? Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_curvature_weight || {}).p50).toFixed(3) : 'N/A'} / ${Number.isFinite(Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_gate_weight || {}).p50)) ? Number((highwayMildCurve.mpc_kappa_active_mild_curve_authority_gate_weight || {}).p50).toFixed(3) : 'N/A'}</td></tr>`;
                html += '</table>';
                html += '</div>';
            }
