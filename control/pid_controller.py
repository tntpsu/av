...
class VehicleController:
    ...
    def __init__(self, lateral_kp: float = 0.3, lateral_ki: float = 0.0, lateral_kd: float = 0.1,
        ...
        self.lateral_controller = LateralController(
            ...
            pp_curve_local_floor_formula_enabled=pp_curve_local_floor_formula_enabled,
            pp_curve_local_floor_target_error_m=pp_curve_local_floor_target_error_m,
            pp_curve_local_floor_speed_time_constant_s=pp_curve_local_floor_speed_time_constant_s,
            pp_curve_local_floor_absolute_min_m=pp_curve_local_floor_absolute_min_m,
            pp_curve_local_floor_curvature_clamp=pp_curve_local_floor_curvature_clamp,
            pp_curve_local_floor_k_reduction_factor=pp_curve_local_floor_k_reduction_factor,
            pp_curve_local_floor_k_severity_kappa_min=pp_curve_local_floor_k_severity_kappa_min,
            pp_curve_local_floor_k_severity_kappa_max=pp_curve_local_floor_k_severity_kappa_max,
            pp_steering_profile_enabled=pp_steering_profile_enabled,
    ...
    def compute_control(
        ...
        if regime == ControlRegime.LINEAR_MPC and self._mpc_controller is not None:
            ...
            if gt_cross_track is not None and gt_heading is not None:
                if _use_lookahead_ref and gt_cross_track_lookahead is not None:
                    # Lookahead reference: same point PP steers toward and scoring
                    # measures. NMPC internal convention matches gt_cross_track
                    # sign directly (no negation needed — opposite of LMPC block).
                    # Verified: LMPC negates and gets corr(mpc_e_lat, lat_err)=-1.
                    # NMPC must also get corr=-1, which requires NO negation here
                    # because the NMPC solver internally handles the sign mapping.
                    raw_e_lat = float(gt_cross_track_lookahead)
                    _mpc_e_lat_ref_source = 'lookahead_gt'
                elif gt_cross_track_source_code == 'road_frame_at_car':
                    raw_e_lat = float(gt_cross_track)
